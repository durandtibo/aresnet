...
def test_validate_retry_params_rejects_negative_max_retries() -> None:
    ...
    with pytest.raises(ValueError, match="max_retries must be >= 0, got -1"):
        validate_retry_params(-1, 0.3)


def test_validate_retry_params_rejects_negative_backoff_factor() -> None:
    """Test that validate_retry_params rejects negative backoff_factor."""
    with pytest.raises(ValueError, match="backoff_factor must be >= 0, got -0.5"):
